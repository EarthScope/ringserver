...
static void    *SL_ListenThread (void *arg);
static void    *DL_ListenThread (void *arg);
...
main (int argc, char* argv[])
{
  ...
  for (;;)
      ...
      while ( loopstp )
	{
	  ...
	  if ( stp->type == DATALINK_LISTEN_THREAD )
	    {
	      threadtype = "DataLink";
	      threadfunc = &DL_ListenThread;
	    }
	  else if ( stp->type == SEEDLINK_LISTEN_THREAD )
	    {
	      threadtype = "SeedLink";
	      threadfunc = &SL_ListenThread;
	    }
	  ...
	  if ( stp->td )
	    {
	      ...
	      if ( stp->td->td_flags & TDF_SPAWNING ) strcat (statusstr, " SPAWNING");
	      if ( stp->td->td_flags & TDF_ACTIVE ) strcat (statusstr," ACTIVE");
	      if ( stp->td->td_flags & TDF_CLOSE ) strcat (statusstr," CLOSE");
	      if ( stp->td->td_flags & TDF_CLOSING ) strcat (statusstr," CLOSING");
	      if ( stp->td->td_flags & TDF_CLOSED ) strcat (statusstr," CLOSED");
	  ...
	  if ( stp->type == DATALINK_LISTEN_THREAD || stp->type == SEEDLINK_LISTEN_THREAD )
	    {
		...
		{
		  lprintf (1, "Joining CLOSED %s listen thread", threadtype);
		  
		  if ( (errno=pthread_join (stp->td->td_id, NULL)) )
		    {
		      lprintf (0, "Error joining CLOSED %s listen thread %lu: %s", threadtype,
	    ...
	    } /* Done with SEEDLINK_LISTEN_THREAD and DATALINK_LISTEN_THREAD handling */
	  else if ( stp->type == MSEEDSCAN_THREAD )
	    {
      ...
      while ( loopctp ) 
	{
	  ...
	  if ( ctp->td->td_flags & TDF_SPAWNING ) strcat (statusstr, " SPAWNING");
	  if ( ctp->td->td_flags & TDF_ACTIVE ) strcat (statusstr," ACTIVE");
	  if ( ctp->td->td_flags & TDF_CLOSE ) strcat (statusstr," CLOSE");
	  if ( ctp->td->td_flags & TDF_CLOSING ) strcat (statusstr," CLOSING");
	  if ( ctp->td->td_flags & TDF_CLOSED ) strcat (statusstr," CLOSED");
	  lprintf (3, "%s", statusstr);
	  ...
	  if ( ctp->td->td_flags & TDF_CLOSED )
	    {
	      ...
	      if ( ! ctp->prev && ! ctp->next ) cthreads = 0;
	      if ( ! ctp->prev && ctp->next ) cthreads = ctp->next;
	      if ( ctp->prev ) ctp->prev->next = ctp->next;
	      if ( ctp->next ) ctp->next->prev = ctp->prev;
 ...
 * DL_ListenThread:
 *
 * Thread to accept connections and dispatch DataLink client threads.
 *
 * Return NULL.
 ***********************************************************************/
void *
DL_ListenThread (void *arg)
{
  pthread_t ctid;
  int clientsocket;
  ...
  lprintf (1, "Listening for DataLink connections on port %s", lpp->portstr);
  ...
  while ( ! shutdownsig )
			...
			NI_NUMERICHOST | NI_NUMERICSERV) )
	{
      ...
      lprintf (2, "Incoming DataLink connection: %s port %s", ipstr, portstr);
      ...
      if ( (cinfo->addr = (struct sockaddr *) malloc (addrlen)) == NULL )
	{
      ...
      strncpy (cinfo->clientid, "DataLink Client", sizeof(cinfo->clientid));
      
      /* Set connection mode */
      cinfo->type = DATALINK_CLIENT;
      ...
      if ( ! (tdp = InitThreadData (cinfo)) )
	{
      ...
      if ( (errno=pthread_create (&ctid, NULL, DL_ClientThread, (void*) tdp)) )
	{
      ...
      else
	{
	  ...
	  pthread_mutex_lock (&cthreads_lock);
	  if ( cthreads ) { ctp->next = cthreads; cthreads->prev = ctp; }
	  else { ctp->next = 0; }
	  cthreads = ctp;
	  pthread_mutex_unlock (&cthreads_lock);
  ...
  lprintf (1, "Listening DataLink thread closing");
  
  return NULL;
}  /* End of DL_ListenThread() */


/***********************************************************************
 * SL_ListenThread:
 *
 * Thread to accept connections and dispatch SeedLink client threads.
 *
 * Return NULL.
 ***********************************************************************/
void *
SL_ListenThread (void *arg)
{
  pthread_t ctid;
  int clientsocket;
  struct thread_data *mytdp;
  ...
  lprintf (1, "Listening for SeedLink connections on port %s", lpp->portstr);
  ...
  while ( ! shutdownsig )
			...
			NI_NUMERICHOST | NI_NUMERICSERV) )
	{
      ...
      lprintf (2, "Incoming SeedLink connection: %s port %s", ipstr, portstr);
      ...
      if ( (cinfo->addr = (struct sockaddr *) malloc (addrlen)) == NULL )
	{
      ...
      /* Set fixed client ID string, clients have no way of reporting an ID */
      strncpy (cinfo->clientid, "SeedLink Client", sizeof(cinfo->clientid));
      
      /* Set connection mode */
      cinfo->type = SEEDLINK_CLIENT;
      ...
      if ( ! (tdp = InitThreadData (cinfo)) )
	{
      ...
      if ( (errno=pthread_create (&ctid, NULL, SL_ClientThread, (void*) tdp)) )
	{
	  lprintf (0, "Error creating new client thread: %s", strerror(errno));
	  if ( clientsocket )
	    close (clientsocket);
  ...
  lprintf (1, "Listening SeedLink thread closing");
  
  return NULL;
}  /* End of SL_ListenThread() */
  ...
  for (optind = 1; optind < argcount; optind++)
    {
	  ...
	  if ( AddServerThread (DATALINK_LISTEN_THREAD, &lpp) )
	    {
	  ...
	  if ( AddServerThread (SEEDLINK_LISTEN_THREAD, &lpp) )
	    {
...
AddServerThread (unsigned int type, void *params)
{
  ...
  if ( type == DATALINK_LISTEN_THREAD || type == SEEDLINK_LISTEN_THREAD )
    {
  ...
  if ( cinfo->reader && cinfo->reader->pktid > 0 )
    {
      ...
      ulatestid = ( ringparams->latestid < ringparams->earliestid ) ?
	ringparams->latestid + ringparams->maxpktid : ringparams->latestid;
      
      upktid = ( cinfo->reader->pktid < ringparams->earliestid ) ?
	cinfo->reader->pktid + ringparams->maxpktid : cinfo->reader->pktid;
