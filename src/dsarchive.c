	  ...
	  *w = '\0';
	  ...
	  switch ( *w )
	    ...
	    case 'n' :
	      ...
	      if ( def ) strncat (definition, msr->network, (sizeof(definition) - fnlen));
	      if ( nondefflags > 0 )
		{
		  if ( def ) strncat (globmatch, msr->network, (sizeof(globmatch) - globlen));
	    ...
	    case 's' :
	      ...
	      if ( def ) strncat (definition, msr->station, (sizeof(definition) - fnlen));
	      if ( nondefflags > 0 )
		{
		  if ( def ) strncat (globmatch, msr->station, (sizeof(globmatch) - globlen));
		  else strncat (globmatch, "*", (sizeof(globmatch) - globlen));
		  globlen = strlen (globmatch);
		}
	      fnlen = strlen (filename);
	    ...
	    case 'l' :
	      ...
	      if ( def ) strncat (definition, msr->location, (sizeof(definition) - fnlen));
	      if ( nondefflags > 0 )
		{
		  if ( def ) strncat (globmatch, msr->location, (sizeof(globmatch) - globlen));
		  else strncat (globmatch, "*", (sizeof(globmatch) - globlen));
		  globlen = strlen (globmatch);
		}
	      fnlen = strlen (filename);
	    ...
	    case 'c' :
	      ...
	      if ( def ) strncat (definition, msr->channel, (sizeof(definition) - fnlen));
	      if ( nondefflags > 0 )
		{
		  if ( def ) strncat (globmatch, msr->channel, (sizeof(globmatch) - globlen));
		  else strncat (globmatch, "*", (sizeof(globmatch) - globlen));
		  globlen = strlen (globmatch);
		}
	      fnlen = strlen (filename);
		...
		{
		  if ( def ) strncat (globmatch, tstr, (sizeof(globmatch) - globlen));
		  else strncat (globmatch, "[0-9][0-9][0-9][0-9][0-9][0-9][0-9]", (sizeof(globmatch) - globlen));
		  globlen = strlen (globmatch);
		}
	      fnlen = strlen (filename);
	      p = w + 1;
		...
		break;
	      ...
	      if ( def ) strncat (definition, hostname, (sizeof(definition) - fnlen));
	      if ( nondefflags > 0 )
		{
		  if ( def ) strncat (globmatch, hostname, (sizeof(globmatch) - globlen));
		  else strncat (globmatch, "*", (sizeof(globmatch) - globlen));
		  globlen = strlen (globmatch);
		}
	      fnlen = strlen (filename);
	      ...
	      char *postpath, int nondefflags, const char *globmatch, char *ident)
{
  ...
  if ( foundgroup == NULL && nondefflags > 0 )
      ...
      if ( rval && rval != GLOB_NOMATCH )
	{
	  switch (rval)
	    {
	    case GLOB_ABORTED : lprintf (1, "[%s] glob(): Unignored lower-level error", ident);
	    case GLOB_NOSPACE : lprintf (1, "[%s] glob(): Not enough memory", ident);
	    case GLOB_NOSYS : lprintf (1, "[%s] glob(): Function not supported", ident);
	    default : lprintf (1, "[%s] glob(): %d", ident, rval);
	    }
	}
